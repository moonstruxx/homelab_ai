...
function ensure_litellm_compat() {
  "$PY" -c "from litellm.utils import validate_and_fix_thinking_param" >/dev/null 2>&1 && return 0
  echo "[litellm] repairing package installation"
  uv pip install -i https://pypi.tuna.tsinghua.edu.cn/simple --extra-index-url https://pypi.org/simple --reinstall --no-cache-dir litellm
}
