    ...
    def equivalent_condition_to_str(self, condition: dict, table_instance=None) -> str | None:
        ...
        def exists(cln):
            ...
            if cln not in columns:
                # Column not present in this table's schema (e.g. code added a
                # new field before infinity_mapping.json/the table caught up) —
                # by definition no row has it, so "field exists" is false for
                # every row. See nuc25.local/CLAUDE.md "Infinity Schema" notes.
                return "1=0"
            ty, de = columns[cln]
