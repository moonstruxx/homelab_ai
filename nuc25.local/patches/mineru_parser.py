...
class MinerUBackend(StrEnum):
    ...
    HYBRID_ENGINE = "hybrid-engine"  # Native MinerU model (local transformers + MPS)
...
class MinerUParser(RAGFlowPdfParser):
    ...
    def check_installation(self, backend: str = "pipeline", server_url: Optional[str] = None) -> tuple[bool, str]:
        ...
        valid_backends = ["pipeline", "vlm-http-client", "vlm-transformers", "vlm-vllm-engine", "vlm-mlx-engine", "vlm-vllm-async-engine", "vlm-lmdeploy-engine", "hybrid-engine"]
    ...
    def _run_mineru_api(
        ...
        # Use async /tasks endpoint: submit, poll, then download result zip.
        # The synchronous /file_parse endpoint times out on long documents.
        task_submit_url = f"{self.mineru_api}/tasks"
        poll_interval = int(os.environ.get("MINERU_POLL_INTERVAL", "5"))
        poll_timeout = int(os.environ.get("MINERU_POLL_TIMEOUT", "7200"))

        try:
            self.logger.info(f"[MinerU] submit async task: {task_submit_url} backend={options.backend} server_url={data.get('server_url')}")
            if callback:
                callback(0.15, f"[MinerU] submit async task: {task_submit_url}")
            with open(pdf_file_path, "rb") as pdf_file:
                ...
                if not task_id:
                    raise RuntimeError(f"[MinerU] no task_id in submit response: {submit_body}")
                self.logger.info(f"[MinerU] task_id={task_id} queued_ahead={submit_body.get('queued_ahead')}")
                if callback:
                    callback(0.20, f"[MinerU] task_id={task_id}")
            ...
            while elapsed < poll_timeout:
                ...
                self.logger.debug(f"[MinerU] task_id={task_id} status={status}")
                if status == "completed":
                    break
                if status == "failed":
                    raise RuntimeError(f"[MinerU] task_id={task_id} failed: {status_body.get('error')}")
                if callback and elapsed % 30 == 0:
                    callback(0.20 + min(0.15, elapsed / poll_timeout), f"[MinerU] task_id={task_id} status={status}")
                time.sleep(poll_interval)
                elapsed += poll_interval
            else:
                raise RuntimeError(f"[MinerU] task_id={task_id} timed out after {poll_timeout}s")

            self.logger.info(f"[MinerU] task_id={task_id} completed, downloading result...")
            if callback:
                callback(0.35, f"[MinerU] downloading result...")
            with requests.get(result_url, timeout=1800, stream=True) as response:
                ...
                if callback:
                    callback(0.40, f"[MinerU] zip file returned, saving to {output_zip_path}...")
                ...
                if callback:
                    callback(0.50, f"[MinerU] Unzip to {output_path}...")
