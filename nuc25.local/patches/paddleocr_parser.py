...
#
# PATCH (2026-06-30): per-page error handling in __images__
# Upstream uses a list comprehension that aborts entirely if any single page
# fails to render. For large PDFs (400+ pages) with a few malformed pages,
# this causes total image generation failure. Fixed by catching per-page
# exceptions and continuing with the remaining pages.
#
...
class PaddleOCRParser(RAGFlowPdfParser):
    ...
    def __images__(self, fnm, page_from=0, page_to=MAXIMUM_PAGE_NUMBER, callback=None):
        ...
        PATCH: per-page error handling — upstream list comprehension aborts
        entirely if any single page fails to render. We catch per-page
        exceptions so that one bad page doesn't kill image generation for
        the whole document.
        """
