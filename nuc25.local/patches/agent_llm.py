...
class LLM(ComponentBase):
    ...
    def __init__(self, canvas, component_id, param: ComponentParamBase):
        ...
        # Patch: don't forward the node's structured-output self-correction retry
        # count (self._param.max_retries, defaults to 0 — see ComponentParamBase) as
        # LLMBundle's HTTP connection-retry budget. That silently overrode the
        # chat_model.py backoff patch's default of 29 down to 0, so every canvas LLM
        # node's connection errors gave up instantly with no retry at all.
    ...
    def _prepare_prompt_variables(self):
        ...
        if self.imgs:
            # Patch: see the same fix in LLM.__init__ above — don't let the node's
            # structured-output retry count (default 0) clobber LLMBundle's HTTP
            # connection-retry budget.
            self.chat_mdl = LLMBundle(self._canvas.get_tenant_id(), model_config)
