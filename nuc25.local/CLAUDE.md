...
- `patches/chat_model.py` — full copy of `rag/llm/chat_model.py` with two fixes, mounted at `/ragflow/rag/llm/chat_model.py:ro`:
  ...
  2. **Exponential backoff with equal jitter, capped at 2h**: `_get_delay(attempt)` (both classes) computes an exponential ceiling `min(RETRY_MAX_DELAY_SECONDS, RETRY_BASE_DELAY_SECONDS * (2 ** (attempt // 2)))` — 500ms start, doubling every 2 attempts (0.5s, 0.5s, 1s, 1s, 2s, 2s, ...), capped at 7200s (2h) — then applies **equal jitter** (AWS "Exponential Backoff And Jitter"): `ceiling/2 + random.uniform(0, ceiling/2)`. Each retry waits between half and all of its rung's ceiling, so growth and the 2h cap are preserved while concurrent retriers are de-synchronized (they no longer re-collide in lockstep against the same overloaded on-device backend, the failure mode plain exponential reintroduced). `LLM_MAX_RETRIES` default raised from 5 to 29 so the schedule reaches the cap: with 29 retries (30 attempts total) the final rung's wait is 1–2h. Total accumulated wait before giving up with `MAX_RETRIES_EXCEEDED` is **~4.9h expected** (3.3h min / 6.55h max). Replaces the earlier flat `base_delay * random.uniform(10, 150)` jitter and the intermediate pure-exponential (no-jitter) form.
