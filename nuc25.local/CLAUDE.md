...
### Daytona Integration (Agentic Sandbox) — scaffolded, not functional, currently stopped

- `daytona-bridge` — intended as a FastAPI bridge providing secure code execution via the Daytona SDK for RAGFlow agents, at `http://daytona-bridge:8000/health`. **As of 2026-07-18, `daytona-bridge/src/main.py` is a stub**: it only calls `Daytona()` and prints a heartbeat in a loop — there is no FastAPI app, no HTTP server, and no `/health` route, so the Gatus check (`gatus/config.yaml`, group `nuc25 — RAG Core`) will fail even once credentials are supplied. `DAYTONA_API_KEY` is also not set in `.env`, so `Daytona()` throws immediately and the container exits after printing `Error: Authentication credentials not found...` — with `restart: always` this crash-looped indefinitely (11 restarts in ~3 min) until stopped.
  - **Two fixes already applied** (found while restoring the stack, see the whole-stack outage note below): the service block had been accidentally appended after the top-level `volumes:` key in `common-docker-compose.nuc25-es-web.yml` instead of under `services:` (broke `docker compose` YAML parsing for the *entire* stack), and `daytona-bridge/requirements.txt` contained the literal text `pip3 install daytona` instead of `daytona` (broke the image build).
  - **Currently `docker compose stop daytona-bridge`'d** — not removed, just stopped, to kill the restart-loop noise. Gatus's "Daytona Bridge" check will correctly show red until this is actually implemented.
  - **To finish this**: `main.py` needs a real FastAPI app (`uvicorn`, bound to `0.0.0.0:8000`) with a `/health` route, plus whatever tool endpoint RAGFlow agents are meant to call for sandboxed code execution; `DAYTONA_API_KEY` needs to be added to `.env` from a real Daytona account. Until then, this is not an active service — don't rely on the description above ("Acts as a tool/MCP server...") as current state.
...
**2026-07-18 entire `rag_ai_stack` down, two config bugs blocked recovery (resolved)**: Found the whole stack stopped — zero `rag_ai_stack-*` containers running or even present (`docker ps -a` showed none), consistent with bash history showing a `docker compose down` run in an earlier session with no subsequent `up`. Data was intact (`srv/stack/{mysql,redis,minio,infinity,langfuse}` untouched), so this was a stopped-stack recovery, not a data-loss incident. Bringing it back up with `docker compose up -d` failed for two independent reasons, both introduced by an in-progress, undocumented `daytona-bridge` scaffold (see "Daytona Integration" above):
1. `common-docker-compose.nuc25-es-web.yml` — the `daytona-bridge` service block had been appended after the top-level `volumes:` key instead of under `services:`, so the whole file failed compose's YAML schema validation (`volumes.daytona-bridge additional properties '...' not allowed`) — this blocked *every* service, not just daytona-bridge, from starting. Fixed by moving the block back under `services:`.
2. `daytona-bridge/requirements.txt` contained the literal shell command `pip3 install daytona` instead of the package name `daytona`, breaking the image build (`ERROR: Invalid requirement: 'pip3 install daytona'`). Fixed.

After both fixes, the whole stack came up healthy except `daytona-bridge` itself, which crash-loops (missing `DAYTONA_API_KEY` + `main.py` has no real HTTP server — see "Daytona Integration" above for the full breakdown) and `gatus`, which panicked on startup (`invalid endpoint _daytona-bridge: you must specify an url for each endpoint`) because the matching Gatus entry in `gatus/config.yaml` used a nonexistent `endpoint:` key instead of `url:` (every other entry in the file uses `url:`) and was missing `conditions:`/`alerts:`. Fixed the Gatus entry to match the standard schema (group `nuc25 — RAG Core`). Per user decision, `daytona-bridge` was left `docker compose stop`'d rather than fixed further, since finishing it means writing a real FastAPI server (out of scope for a stack-recovery task) and supplying a real Daytona API key. All other services confirmed healthy via `curl http://localhost:8090/api/v1/endpoints/statuses` after the fix.
